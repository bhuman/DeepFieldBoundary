...
def train(performance, directories, settings, checkpoint):
    ...
    dataset_path = os.path.join(os.path.dirname(__file__), '..', f'{directories["data_basedir"]}', f'{settings["dataset"]}')
    ...
    callbacks.TensorboardImageLogger(log_dir=os.path.join(os.path.dirname(__file__), "..", f"{checkpoint['checkpoint_dir']}", "tensorboard", "input")).log_images(train_generator[0][0] if settings['grayscale'] else np.asarray([color_conversion.yuv2rgb(img) for img in train_generator[0][0]]), name='Training first batch', epoch=checkpoint['epoch'])
    callbacks.TensorboardImageLogger(log_dir=os.path.join(os.path.dirname(__file__), "..", f"{checkpoint['checkpoint_dir']}", "tensorboard", "input")).log_images(val_generator[0][0] if settings['grayscale'] else np.asarray([color_conversion.yuv2rgb(img) for img in val_generator[0][0]]), name='Validation first batch', epoch=checkpoint['epoch'])
    ...
    keras.utils.plot_model(model, to_file=os.path.join(os.path.dirname(__file__), "..", f"{checkpoint['checkpoint_dir']}", f"{settings['model']}.png"), show_shapes=True, show_layer_names=True, rankdir='TB')
    model.summary()
    ...
    train_callbacks = [
        ...
        TensorBoard(log_dir=os.path.join(os.path.dirname(__file__), "..", f"{checkpoint['checkpoint_dir']}", "tensorboard"), histogram_freq=1, write_graph=True),
        TerminateOnNaN(),
